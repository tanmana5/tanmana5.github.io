...
This is not necessarily a strict sequence where every model replaces the previous one; they solve somewhat different problems. Thus, several complementary developments are happening:
...
> 
And that is arguably the more interesting direction for robotics: rather than making the policy itself responsible for safety, we may be able to give the robot the ability to imagine possible futures and explicitly check them before acting.